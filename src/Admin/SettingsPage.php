...
final class SettingsPage {
	...
	public function render(): void {
		...
		$allow_engines = $this->settings->allow_search_engines();
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table" role="presentation">
					...
					<tr>
						<th scope="row">
							<?php esc_html_e( 'Known search engines', 'simple-x402' ); ?>
						</th>
						<td>
							<fieldset>
								<?php // Hidden companion input so unchecking the box submits a value. ?>
								<input
									type="hidden"
									name="<?php echo esc_attr( $option ); ?>[allow_search_engines]"
									value="0"
								/>
								<label>
									<input
										...
										name="<?php echo esc_attr( $option ); ?>[allow_search_engines]"
										value="1"
										<?php checked( $allow_engines, true ); ?>
									/>
									<?php esc_html_e( 'Let Googlebot, Bingbot, and other known search indexers through', 'simple-x402' ); ?>
								</label>
								<p class="description">
									<?php esc_html_e( 'Only applies when paywalling bots. Search-engine indexers (Googlebot, Bingbot, DuckDuckBot, YandexBot, Baiduspider) bypass the paywall so your posts stay indexable. AI and agent crawlers (e.g. GPTBot, ClaudeBot, PerplexityBot, Google-Extended) are still paywalled.', 'simple-x402' ); ?>
								</p>
							</fieldset>
						</td>
					</tr>
