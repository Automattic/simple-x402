...
( function () {
	if ( ! window.x402Pay || typeof window.x402Pay.registerProvider !== 'function' ) {
		console.error( '[x402-pay] gravatar-wallet provider loaded before host; skipping.' );
		return;
	}
	...
	window.x402Pay.registerProvider( 'gravatar-wallet', function ( host ) {
		var gravatarOrigin = host.config.gravatarOrigin;
		if ( host.requirements.network !== 'base' ) {
			console.warn(
				'[x402-pay] Gravatar Wallet only signs USDC on Base mainnet. ' +
		...
		button.innerHTML = ''
			+ '<span class="x402-pay-icon" aria-hidden="true">'
			...
			+   '</svg>'
			+ '</span>'
			+ '<span class="x402-pay-label">Pay with Gravatar Wallet</span>';
